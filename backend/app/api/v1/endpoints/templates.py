...
def publish_template(
    ...
    if body.source_emotion not in VALID_EMOTIONS:
        raise HTTPException(
            status_code=422, detail=f"Invalid source_emotion: {body.source_emotion}"
        )
    if body.target_emotion not in VALID_EMOTIONS:
        raise HTTPException(
            status_code=422, detail=f"Invalid target_emotion: {body.target_emotion}"
        )
...
def list_templates(
    ...
    total = q.count()
    items = (
        q.order_by(ArcTemplate.remix_count.desc(), ArcTemplate.created_at.desc())
        .offset(offset)
        .limit(limit)
        .all()
    )
    ...
    return {
        "total": total,
        "templates": [
            _template_dict(t, author_names.get(str(t.user_id), "")) for t in items
        ],
    }
...
def remix_template(
    ...
    return {
        ...
        "mood_interpretation": (
            tmpl.description
            or f"Remixed: {tmpl.source_emotion} → {tmpl.target_emotion}"
        ),
        "source_emotion": tmpl.source_emotion,
        "target_emotion": tmpl.target_emotion,
